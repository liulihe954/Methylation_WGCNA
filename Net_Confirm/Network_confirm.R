...
data_expr_all = data_expr_all[,c(which(substr(raw_data_index,2,5) %in% control_index),
                                 which(substr(raw_data_index,2,5) %in% treatment_index))]

setwd("/ufrc/penagaricano/lihe.liu/Methylation_WGCNA/Net_Confirm")
networkData_final  =  DataPre_C(data_expr_all, cousin = 0.4, n1 = 9, n2 = 10, perct = 0.5) # select top 20 (1-0.8)
...
#######set new labels##############
##################################
control_index_confirm = c(treatment_index[1:5],control_index[6:10])
treatment_index_confirm = c(control_index[1:5],treatment_index[6:10])
# new structure
datExpr_control = t(network_final[,which(substr(names(network_final),2,5) %in% control_index_confirm)])
datExpr_treatment = t(network_final[,which(substr(names(network_final),2,5) %in% treatment_index_confirm)])
